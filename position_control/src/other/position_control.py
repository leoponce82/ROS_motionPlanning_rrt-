	...
	if (not len(waypoints_list)==False) and callback_waypoints.end==False: #True si es q la lista esta vacia
		for each_coord in waypoints_list:
			...
			while ( dist(a_point,each_coord) > 10 ):
				...
				"""
				# INTENDO DE CONTROL con mat rotacional
				matA=[[1,0],[0,1]]
				matR=[[cos(-error) , -sin(-error)] , [sin(-error) , cos(-error)]]
				matC=[[each_coord[0]],[each_coord[1]]]
				AxR = np.dot(matA,matR) 
				AxRxC= np.dot(AxR,matC)
			
				print(AxRxC)
				print("error:",error)
				velocidad_cte=AxRxC[0][0]
				velocidad_w=AxRxC[1][0]
