...
def callback_colisiones(data):
	#print(data)
	callback_colisiones.id_colision=data.aruco_id
	callback_colisiones.cmd_stop=data.cmd_stop #True si es q debe detenerse
	#print(callback_colisiones.id_colision)
	#print(callback_colisiones.cmd_stop)
	


class control_exe(multiprocessing.Process):
	global aruco_id_worker
	global quit_flag
		...
		while not self.exit.is_set():
			if  quit_flag==False:
				...
				for each_coord in all_waypoints_list[id_aruco][0]:
					...
					try:
						controlled_index = read_ids().index([id_aruco])#Para cotrolar el robot con ID 4
					except ValueError: 
					...
					while ( dist(a_point,each_coord) > 10 ) and (not self.exit.is_set()):
						if quit_flag==False:
							os.system('clear')
							#print(str(XBEE_ADDR_LONG))
							...
							print("error:",error)
							print("angulo aruco:",angulo_actual)
							...
							if vl <= -32767:
								vl=-32767
							"""
							if callback_colisiones.id_colision == id_aruco and callback_colisiones.cmd_stop == True:
...
def control_exe(id_aruco):
	...
	xbee.tx(dest_addr_long = XBEE_ADDR_LONG, dest_addr = XBEE_ADDR_SHORT, data=data1,)
		...
		data1 = struct.pack('hhh',2,0,0) #2 es para setear velocidades
		xbee.tx(dest_addr_long = XBEE_ADDR_LONG, des--t_addr = XBEE_ADDR_SHORT, data=data1,)
		...
		if each_list[0]!=0:
			for n in range(len(each_list[0])):
				...
				if n<( (len(each_list[0]))-1 ):
					...
					cv2.circle(callback_imagen.image_np,(int(each_list[0][n][0]), int(each_list[0][n][1])), 2, (255,0,0), -1)


	"""
	################# DIBUJA CENTROS  de ARUCO #############
	try:
		aruco_centerback_draw=tuple ( (int(control_exe.aruco_centerback[0]),int(control_exe.aruco_centerback[1])) )
		a_point=(int(control_exe.x), int(control_exe.y))
		...
		#print('No se ha detectado aruco ID=4_ IMAGEN PROCESS')
		pass
...
def position_control_all():
	...
	class control(qtw.QMainWindow,Ui_control): #EL PRIMER ARGUMENTO DEBE COINCIDIR CON LO ESCOGIDO EN DESIGNER
		id_aruco_signal1=qtc.pyqtSignal(int)
		id_aruco_signal2=qtc.pyqtSignal(int)
		id_aruco_signal3=qtc.pyqtSignal(int)
		id_aruco_signal4=qtc.pyqtSignal(int)
		def __init__(self, *args, **kwargs):
			...
			self.id_aruco_signal1.connect(self.exe_control1)
			self.id_aruco_signal2.connect(self.exe_control2)
			self.id_aruco_signal3.connect(self.exe_control3)
			self.id_aruco_signal4.connect(self.exe_control4)
		...
		def ejecutar_control1(self):
			global aruco_id_worker
			aruco_id_worker=1
			self.id_aruco_signal1.emit(1)
				
		def exe_control1(self):
		...
		def ejecutar_control2(self):
			global aruco_id_worker
			aruco_id_worker=2
			self.id_aruco_signal2.emit(2)
				
				
		def exe_control2(self):
		...
		def ejecutar_control3(self):
			global aruco_id_worker
			aruco_id_worker=3
			self.id_aruco_signal3.emit(3)
				
				
		def exe_control3(self):
		...
		def ejecutar_control4(self):
			global aruco_id_worker
			aruco_id_worker=4
			self.id_aruco_signal4.emit(4)
				
				
		def exe_control4(self):
		...
		def ejecutar_control0(self):
			...
			self.id_aruco_signal1.emit(1)
			aruco_id_worker=2
			self.id_aruco_signal1.emit(2)
			aruco_id_worker=3
			self.id_aruco_signal1.emit(3)
			aruco_id_worker=4
			self.id_aruco_signal1.emit(4)
		def stop_all(self):
